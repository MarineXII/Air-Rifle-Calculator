...
type
  TForm1 = class(TForm)
    ...
    procedure btnFpsMsClick(Sender: TObject);
    procedure btnMsToFpsClick(Sender: TObject);
...
procedure TForm1.btnFpsMsClick(Sender: TObject);
  var fps, ms : real;
      mss : string;
      fpi : integer;
begin
  result.Clear;
  Val(fpstoms.Text, fps, fpi);
  ms := fps*0.3048;
  mss := Format('%.5f', [ms]);
  result.Lines.Add(FloatToStr(fps) + 'ft/s is ' + mss + 'm/s.');
end;

procedure TForm1.btnMsToFpsClick(Sender: TObject);
  var ms, fps : real;
      fpss : string;
      msi : integer;
begin
  result.Clear;
  Val(mstofps.Text, ms, msi);
  fps := ms/0.3048;
  fpss := Format('%.5f', [fps]);
  result.Lines.Add(FloatToStr(ms) + 'm/s is ' + fpss + 'ft/s');
end;
